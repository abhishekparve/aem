...
/**
 * The Class BDBProcessBulkLegacyDocJobTest.
 */
@ExtendWith({ MockitoExtension.class })
class BDBProcessBulkLegacyDocJobTest {

	/** The process product document. */
	@InjectMocks
	BDBProcessBulkLegacyDocJob bDBProcessBulkLegacyDocJob;

	/** The process product document service. */
	@Mock
	private ProcessProductDocumentService processProductDocumentService;
	...
	void processExceptionTest() throws WorkflowException, LoginException, RepositoryException,
		...
		when(resourceResolverFactory.getServiceResourceResolver(Mockito.anyMap())).thenReturn(resourceResolver);
		when(resourceResolver.adaptTo(Session.class)).thenReturn(session);
		doThrow(new InvalidTagFormatException("Exception")).when(processProductDocumentService)

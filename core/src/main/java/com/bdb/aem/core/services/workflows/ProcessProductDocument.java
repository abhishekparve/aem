...
@Component(immediate = true, service = WorkflowProcess.class, property = {
		"process.label = Product documents processing workflow" })
public class ProcessProductDocument implements WorkflowProcess {

	/** The Constant LOG. */
	private static final Logger LOGGER = LoggerFactory.getLogger(ProcessProductDocument.class);
	...
	public void execute(WorkItem workItem, WorkflowSession workflowSession, MetaDataMap args) throws WorkflowException {
		LOGGER.debug("Entry Execute method of ProcessProductDocument");
		...
		try {
			...
			if (null != session) {
				...
				if (StringUtils.isNotBlank(payload) && (workflowHelperService.canProcessPath(payload, bDBWorkflowConfigService.getDocBasePath())
					...
					List<String> listOfAssets = workflowHelperService.getListOfPDFAssets(payload, serviceResolver,
							session);
					workflowHelperService.assignJobInBatches(listOfAssets, jobManager, "legacy-Doc-Workflow");
				}
			}
		...
		LOGGER.debug("Exit Execute method of ProcessProductDocument");
	}
}
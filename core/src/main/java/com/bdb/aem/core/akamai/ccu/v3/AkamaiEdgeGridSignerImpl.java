...
public class AkamaiEdgeGridSignerImpl implements AkamaiEdgeGridSigner {
	 ...
	 */
	private static final Logger LOG = LoggerFactory.getLogger(AkamaiEdgeGridSignerImpl.class);
	...
	public AkamaiEdgeGridSignerImpl(List<String> headers, int maxBodySize) {
		this.headersToInclude = Collections.unmodifiableList(headers) ;
			...
			if (httpContent != null) {
				try {
					...
					if (lengthToHash > maxBodySize) {
						LOG.warn("Message body length {} is larger than the max {}. Using {} bytes for computing the hash.", lengthToHash, maxBodySize, maxBodySize);
						lengthToHash = maxBodySize;
					} else {
						if (LOG.isDebugEnabled()) {
							LOG.debug(String.format("Content: %s", Base64.encodeBase64String(contentBytes)));
						}
					}
					...
					if (LOG.isDebugEnabled()) {
						LOG.debug(String.format("Content hash: %s", Base64.encodeBase64String(digestBytes)));
					}
	...
	public HttpRequestBase sign(HttpRequestBase request, ClientCredential credential) throws AkamaiException {
		...
		byte[] signingKeyBytes = sign(timeStamp, clientSecret.getBytes(StandardCharsets.UTF_8), HMAC_ALG);
		...
		byte[] signatureBytes = sign(stringToSign, signingKey.getBytes(StandardCharsets.UTF_8), HMAC_ALG);
